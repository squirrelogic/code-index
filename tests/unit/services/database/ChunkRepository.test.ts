...
function createTestDatabase(): Database.Database {
  ...
  db.exec(`
    ...
    -- FTS5 virtual table (standalone, not content= table)
    ...
    CREATE TRIGGER IF NOT EXISTS chunks_fts_delete AFTER DELETE ON chunks BEGIN
      DELETE FROM chunks_fts WHERE rowid = (SELECT rowid FROM chunks_fts WHERE chunk_id = old.id);
    END;
    ...
    CREATE TRIGGER IF NOT EXISTS chunks_fts_update AFTER UPDATE ON chunks BEGIN
      ...
      WHERE rowid = (SELECT rowid FROM chunks_fts WHERE chunk_id = new.id);
    END;
  `);
...
function createTestChunk(overrides?: Partial<{
  ...
  return new Chunk(
    randomUUID(),
    'a'.repeat(64), // Simple hash for testing
