...
extract_modified_files() {
    ...
    case "$tool_name" in
        Write|Edit|MultiEdit)
            ...
            ;;
        NotebookEdit)
            # Extract notebook_path directly from the hook event JSON
            local notebook_path=$(echo "$HOOK_EVENT_JSON" | jq -r '.tool_input.notebook_path // ""' 2>/dev/null)
...
trigger_index_refresh() {
...

    {
        ...
        # Build refresh command with file paths if known
        local refresh_cmd="code-index refresh"
        if [ "$modified_files" != "<multiple>" ] && [ -n "$modified_files" ]; then
            # Pass specific file(s) to refresh command
            refresh_cmd="code-index refresh $modified_files"
        fi

        # Run refresh command
        if $refresh_cmd 2>&1 | while IFS= read -r line; do
